...
void pack_A_safe_opt(int mc, int kc, const uint8_t* A, int k_bytes, uint8_t* A_packed) {
...
void pack_B_safe_opt(int kc, int nc, const uint8_t* B, int m, uint8_t* B_packed) {
...
void micro_kernel_final_opt(int kc, const uint8_t* A_p, const uint8_t* B_p, int* C, int m, int cur_mc, int cur_nc) {
    __m256i acc[4][4]; 
    for(int i=0; i<4; ++i) { 
...
void gemmV14_BLIS_SingleThread_Optimized(uint8_t* Anew, uint8_t* Bnew, int* resV1, int n, int m, int k) {
    ...
    for (int jc = 0; jc < m; jc += NC) {
        ...
        for (int pc = 0; pc < k_bytes; pc += KC) {
            ...
            pack_B_safe_opt(cur_kc, cur_nc, &Bnew[pc * m + jc], m, B_packed);

            for (int ic = 0; ic < n; ic += MC) {
                int cur_mc = std::min(MC, n - ic);

                pack_A_safe_opt(cur_mc, cur_kc, &Anew[(ic * k_bytes + pc) * 2], k_bytes, A_packed);
